...
REMOVED_FIELDS = frozenset({
    "task_name", "task_id", "fork_turns", "host_binding", "allocation", "admission",
    "dispatch_id", "receipt", "receipt_id", "prior_hard_stop", "candidate_changed_paths",
    "critical", "native_collaboration",
})
...
def format_native_payload(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if "fork_context" in packet and packet["fork_context"] is not False:
        errors.append("fork_context: must be false when supplied")
    if errors:
    ...
    payload: dict[str, Any] = {"agent_type": agent_type, "fork_context": False, "message": message}
